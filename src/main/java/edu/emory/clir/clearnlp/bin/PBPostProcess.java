...
public class PBPostProcess {
    ...
    final private Pattern TRACE = Pattern.compile("\\*T\\*-\\d+");
    ...
    /**
     * @param instances list of PropBank instances that have been associated with constituency trees
...
//			System.out.println(instance.getKey());
                ...
                // if a tag follows
                    ...
                    for (CTNode child : curr.getChildrenList()) {
                        ...
                        for (CTNode subChild : child.getChildrenList()) {
                            if (subChild.getPBLocation().getTerminalID() != child.getPBLocation().getTerminalID()) {
                                continue;
                            }
                            if (subChild.isEmptyCategory() && subChild.matchesWordForm(TRACE)) {
                                if (!(list = subChild.getEmptyCategoryListInSubtreeCycleFree(P_NONE)).isEmpty()) {
                                    cLoc.setLocation(child.getPBLocation());
                                    for (CTNode ec : list) {
                                        lDel.add(new PBLocation(ec.getPBLocation(), ""));
                                        if ((ante = ec.getAntecedent()) != null) {
    ...
    /**
     * Called by {@link PBPostProcess#normalizeLinks(PBInstance)}.
     */
    ...
    private boolean findOverlappingArguments(PBInstance instance) {
        ...
        List<PBArgument> lDel = new ArrayList<>();
...
//					System.err.println(build.toString());
    ...
    private PBArgument getArgumentDSP(PBInstance instance) {
        ...
        Pair<PBArgument, Set<Integer>> max = new Pair<>(null, new HashSet<>());
        CTNode prn = pair.o1;
        CTNode esm = pair.o2;
        Set<Integer> set;
    ...
    private Pair<CTNode, CTNode> getESMPair(CTNode pred) {
        ...
        if (s != null && s.getParent().isConstituentTag(CTLibEn.C_PRN)) {
            ...
            if (next != null) {
                ...
                if (ec != null) return new Pair<>(s.getParent(), ec);
            }
        }
